...
public abstract class NaturalSatellite extends CelestialBody {
    ...
    public void setBodyPropByName(String prop, String value) {
        switch (prop) {
            case "name":
            case "Name":
            ...
            case "Central Celestial Body":
                ...
                for (int i = 0; i < planetSystemList.size(); i++) {
                    if (planetSystemList.get(whatSystem).getCenterStar().getName().equals(value)) {

                        this.centralCelestialBody = planetSystemList.get(whatSystem).getCenterStar();
                    } else if (planetSystemList.get(whatSystem).bodyExists(value, PlanetSystem.Body.PLANET)) {

                        this.centralCelestialBody = planetSystemList.get(whatSystem).getPlanet(value);
                    } else {
                        System.out.println("did not find a body with that name");
                    }
                }
